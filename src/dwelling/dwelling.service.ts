...
export class DwellingService {
  ...
  async updateOne (dwellingId: number, image: any, updateData: UpdateDwellingDto, userId: number) {
    ...
    if (updateData.name) { dwelling.name = updateData.name }
    if (updateData.price) { dwelling.price = updateData.price }
    if (image) {
      const imageFileName: string = await this.filesService.createFile(image)
      dwelling.image_path = imageFileName
    }
    if (updateData.tagIds) { dwelling.tags = await this.tagService.getTagListByIds(JSON.parse(updateData.tagIds)) }
    if (updateData.streetId) { dwelling.street = await this.streetService.getOne(updateData.streetId) }
    return await this.dwellingRepository.save(dwelling)
  }
  ...
  async search (searchDto: SearchDto): Promise<Dwelling[]> {
    ...
    if (searchDto.minPrice) { queryBuilder.andWhere('dwelling.price > :minPrice', { minPrice: searchDto.minPrice }) }
    if (searchDto.maxPrice) { queryBuilder.andWhere('dwelling.price < :maxPrice', { maxPrice: searchDto.maxPrice }) }
    if (searchDto.countryId) { queryBuilder.andWhere('dwelling.country_id = :countryId', { countryId: searchDto.countryId }) }
    if (searchDto.cityId) { queryBuilder.andWhere('dwelling.city_id = :cityId', { cityId: searchDto.cityId }) }
    if (searchDto.streetId) { queryBuilder.andWhere('dwelling.street_id = :streetId', { streetId: searchDto.streetId }) }
    if (searchDto.updatePeriod) { queryBuilder.andWhere(`dwelling.updated_at < now() and dwelling.updated_at > now() - INTERVAL '${searchDto.updatePeriod} days'`) }
